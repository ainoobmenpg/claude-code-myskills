...
| `/mysk-spec-implement` | 実装計画を作成（計画のみ、確度付き形式を出力） | メイン | `[run_id]` |
| `/mysk-implement-start` | impl-plan.mdを読み込み実装を実行（impl-planを補完しながら実装） | メイン | `[run_id]` |
...
| `/mysk-spec-implement` | `spec.md` | `impl-plan.md`（確度付き形式） | なし |
| `/mysk-implement-start` | `impl-plan.md`, `spec.md`（補助参照） | プロジェクトコードの変更（myskファイルは更新しない） | なし |
