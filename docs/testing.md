...
| レイヤ | 主なファイル | 目的 |
|--------|-------------|------|
| 静的契約 | `tests/unit/frontmatter.bats`, `tests/unit/template-vars.bats`, `tests/unit/json-schema.bats` | frontmatter、テンプレート変数、JSON ブロック例の破損を防ぐ |
...
| 場所 | 用途 |
|------|------|
| `tests/helpers/test-common.bash` | コマンド一覧、テンプレート一覧、frontmatter、テンプレート変数抽出の共通処理 |
