...
mysk は、初心者向けに公開フローを `仕様策定 -> Issue 作成 -> 実装 -> レビュー` の 4 段階へ絞ったスキル集です。公開コマンドは 6 個だけで、runtime は `commands/` と `templates/mysk/*.md` で完結します。旧コマンド定義は `templates/mysk/legacy-commands/` に archive されています。

`/mysk-help` も公開コマンドだが、表示内容は実運用の 5 コマンド (`/mysk-spec`、`/mysk-issue`、`/mysk-implement`、`/mysk-review`、`/mysk-reset`) を中心に要約する。
...
| `/mysk-issue` | spec.md をベースに GitHub Issue 作成 | `[run_id]` |
...
Issue 作成時は必ず優先度ラベルを付与する。

| 優先度 | タイトルプレフィックス | ラベル | 基準 |
|--------|----------------------|--------|------|
| P0 | `[P0]` | `priority: high` | 実運用 blocker。コマンドが動作しない、データ損失 |
| P1 | `[P1]` | `priority: medium` | 通常のバグ・機能要望 |
| P2 | `[P2]` | `priority: low` | 改善案、余裕があれば対応 |
| P3 | `[P3]` | `priority: low` | 将来の検討課題 |

### タイプラベル

優先度に加えて、以下のいずれかのタイプを付与する。

- `bug`: バグ修正
- `enhancement`: 新機能または機能改善
