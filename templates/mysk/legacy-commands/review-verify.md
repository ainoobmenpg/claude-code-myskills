...
- RUN_DIR を明示的に設定:
  ...
  ```
- `SPEC_PATH="$RUN_DIR/spec.md"` として扱う（存在する場合のみ verify の追加コンテキストに使う）
...
2. 初回実行（TRUTH_PATH なし）:
   - bashで設定:
     ...
     echo "VERIFY_JSON_PATH=$VERIFY_JSON_PATH"
     ```
3. 過去結果あり（TRUTH_PATH が存在）:
   - `jq` で TRUTH_PATH の `verification_result` を取得
   - `result == "passed"` → **TEST MODE**: 環境変数 MYSK_TEST_MODE が "1" の場合、AskUserQuestion をスキップし自動的に承認として扱う。そうでない場合は AskUserQuestion でユーザー確認（自然な対話）
     - 承認 → bashで設定:
       ```bash
       VERIFY_JSON_PATH="$RUN_DIR/verify-rerun.json"
       echo "VERIFY_JSON_PATH=$VERIFY_JSON_PATH"
       ```
   - `result != "passed"` → bashで設定:
     ```bash
     VERIFY_JSON_PATH="$RUN_DIR/verify-rerun.json"
     echo "VERIFY_JSON_PATH=$VERIFY_JSON_PATH"
     ```
...
```bash
GRACE_FILE=$(dirname "$VERIFY_JSON_PATH")/timeout-grace.json
...
| non-high（medium/low）のみ | /mysk-review-fix に戻る |
