...
```bash
GRACE_FILE=$(dirname "$REVIEW_JSON_PATH")/timeout-grace.json
MARKER_FILE=$(dirname "$REVIEW_JSON_PATH")/review-marker.txt
sed -e "s|{REVIEW_JSON_PATH}|$REVIEW_JSON_PATH|g" -e "s|{RUN_ID}|$RUN_ID|g" -e "s|{WS_REF}|$WS_REF|g" -e "s|{SUB_SURFACE}|$SUB_SURFACE|g" \
  -e "s|{GRACE_FILE}|$GRACE_FILE|g" -e "s|{MARKER_FILE}|$MARKER_FILE|g" \
