...
- `spec.md` の `タスク種別` が `docs/text-only` の場合、acceptance snapshot と Changed Paths / Diff Stat / Diff Patch を primary context とし、必要時だけ最小確認対象へ広げてください。それ以外の場合は従来通り repo 全体を確認してください。
...
2. **進捗更新**: 各指摘の検証完了時に `updated_at` を更新してください
   ...
   **重要**: `phase` は進行状況に応じて更新してください:
   - `loading`: 入力の読み込み中
   - `checking_acceptance`: 受け入れ条件の検証中
   - `checking_findings`: 指摘の検証中
   - `writing`: 検証結果の書き込み中
   - `completed`: 検証完了
