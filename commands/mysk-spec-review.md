...
if [ -n "$ARGUMENTS" ]; then
  ...
  # カレントプロジェクトのWORK_DIRを取得
  WORK_DIR=$(git rev-parse --show-toplevel 2>/dev/null || pwd)

  # WORK_DIRに一致するproject_rootを持つ最新のrun_idを自動選択
  CANDIDATES=$(ls -t ~/.local/share/claude-mysk/)
  SELECTED_RUN_ID=""
  ...
  for CANDIDATE in $CANDIDATES; do
    ...
    # review.jsonまたはspec-review.jsonでproject_rootを確認
    if [ -f "$CANDIDATE_DIR/review.json" ]; then
      PROJECT_ROOT=$(jq -r '.project_root // empty' "$CANDIDATE_DIR/review.json" 2>/dev/null)
    elif [ -f "$CANDIDATE_DIR/spec-review.json" ]; then
      PROJECT_ROOT=$(jq -r '.project_root // empty' "$CANDIDATE_DIR/spec-review.json" 2>/dev/null)
    fi

    # WORK_DIR と一致する場合、最新（ls -tの順）のものを選択
    if [ -n "$PROJECT_ROOT" ] && [ "$PROJECT_ROOT" = "$WORK_DIR" ]; then
